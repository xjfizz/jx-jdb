	...
	<view class="widthAuto">
		...
		<view class="widthAutoInner">
			...
			<view class="vip-lb">
				...
				<view class="vip-lb-da"><input disabled  class="vip-lb-input" type="number" value="" placeholder="输入手机号" v-model="retUserPhone"
					  /></view>
			</view>
			<view class="vip-lb">
				<view class="vip-lb-money">用户身份</view>
				<view class="vip-lb-da">
					<view class="status-select" v-bind:class="[selectIndex == 1 ? 'select-ok' : '']">
						<img src="/static/img/selected@2x.png" alt="" class="check_choose" />
						个人</view>
					<view class="status-select" v-bind:class="[selectIndex == 0 ? 'select-ok' : '']">
						<img src="/static/img/selected@2x.png" alt="" class="check_choose" />
						企业</view>
			...
			<view class="vip-lb">
				...
				<view class="vip-lb-da">
					<input disabled class="vip-lb-input" type="text" value="" placeholder="输入用户名称" v-model="retUserWhatCall" />
				</view>
			</view>
			<view class="vip-lb">
				...
				<view class="vip-lb-da">
					<picker disabled :value="index" :range="array">
						<view class="uni-input">{{ retUser.supplierIndustry||array[index] }}</view>
					</picker>
				</view>
			</view>
			<view class="vip-lb">
				...
				<view class="vip-lb-da" style="width: 450upx;">
					...
					<h2 v-else class="uni-input">{{ retUser.supplierAddress }}</h2>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			getUserData() {
				...
				this.API.service.getToken(`${this.URL.getUser}/${params.pkid}`).then(res => {
					if (res.code == 200) {
						this.retUser = res.data;
						this.memberImg = this.retUser.supplierAvatar
						this.retUserWhatCall = this.retUser.supplierNickname;
						this.retUserPhone = this.retUser.supplierPhone;
						this.selectIndex = this.retUser.supplierIdentity;
					}
				})
