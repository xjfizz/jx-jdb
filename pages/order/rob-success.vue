<template>
	<view class="page">
		...
		<uni-popup ref="popupU" type="uploadProgress">
		...
		<view class="main">
			...
			<!-- 		<view v-if="orderDetail.orderStatus == 4 || orderDetail.orderStatus == 5 || orderDetail.orderStatus == 6|| orderDetail.orderStatus == 7 "
					 class="production">
					...
					<view class="mid-item-product">
						...
						<view class="product-item">
							...
							<view class="product-value">{{orderDetail.orderTotalQuantity}}</view>
						</view>
						<view class="product-item">
...
<script>
	...
	export default {
		computed: {
			...
			loading() {
				return false // this.$store.state.loadingShow
			}
		},
		data() {
			...
			return {
				...
				orderStatusControl: [{
					...
					},
					{
						status: 5,
						statusText: '抢单成功...',
						statusTips: ' 您已抢单成功，去联系平台吧',
						statusImg: '../../static/img/order/order-status-5-success.png',
					},
					{
						status: 6,
						statusText: '订单正在进行中...',
		...
		methods: {
			...
			getDetail() {
				let params = {
					...
					orderStatus:5, // this.orderDetail.orderStatus,抢单成功
			...
			submitProgress(des) {
				// TODO 做一些其他的事情，手动执行 done 才会关闭对话框
				console.log('submitProgress', des)
