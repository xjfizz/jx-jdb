...
// 格式化银行卡号
let formatBankNo = function(cardNumber) {
...
// 银行卡号安全-格式化
let bankNoSafe = function(val) {
	// return val.replace(/\s/g,'').replace(/(\d{4})\d+(\d{4})$/, "**** **** **** $2") // **** **** **** 6598
	 if (val && val.length > 8) {
	        return `${val.substring(0, 4)} ${"*".repeat(val.length - 8).replace(/(.{4})/g, `$1 `)}${val.length % 4 ? " " : ""}${val.slice(-4)}`;
			//return `${"*".repeat(cardNumber.length - 4).replace(/(.{4})/g, `$1 `)}${cardNumber.length % 4 ? " " : ""}${cardNumber.slice(-4)}`;
		}
	    return val;
};
...
// 手机号安全-格式化
let phoneNumberSafe = function(val) {
	// return val.replace(/\s/g,'').replace(/(\d{4})\d+(\d{4})$/, "**** **** **** $2") // **** **** **** 6598
	val = val.toString()
	 if (val && val.length > 8) {
...
// 身份证安全-格式化
let userIdCardSafe = function(val) {
	// return val.replace(/\s/g,'').replace(/(\d{4})\d+(\d{4})$/, "**** **** **** $2") // **** **** **** 6598
	val = val.toString()
	 if (val && val.length > 8) {
